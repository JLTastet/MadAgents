...
def _mask_content(content, max_chars: int):
    """Truncate content keeping head and tail for context."""
    if isinstance(content, str):
        ...
        return (
            content[:half]
            + f"\n[... {len(content) - max_chars} chars masked ...]\n"
            + content[-half:]
        )
    if isinstance(content, list):
        new_blocks = []
                    ...
                    if isinstance(val, str) and len(val) > max_chars:
                        ...
                        b = {
                            **b,
                            key: (
                                val[:half]
                                + f"\n[... {len(val) - max_chars} chars masked ...]\n"
                                + val[-half:]
                            ),
                        }
                        changed = True
...
def _rebuild_tool_message(m: BaseMessage, new_content) -> BaseMessage:
    """Reconstruct a tool-result message with new content."""
    if isinstance(m, ToolMessage):
        return ToolMessage(
            content=new_content,
    ...
    try:
        ...
        return cls(content=new_content)
...
def mask_tool_observations(
    ...
    *,
    ...
    """Mask verbose tool-result content while preserving tool call structure.

    Keeps head+tail of each tool output so the worker retains enough detail
    from past invocations.  Non-tool-result messages are passed through
    unchanged.
    """
    ...
    for m in messages:
        ...
        content = getattr(m, "content", "")
        masked = _mask_content(content, max_result_chars)

        if masked is content:
            out.append(m)
            continue

        out.append(_rebuild_tool_message(m, masked))
    return out
