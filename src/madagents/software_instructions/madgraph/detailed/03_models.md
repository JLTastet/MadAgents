...
MadGraph5 uses physics models in the UFO (Universal FeynRules Output) format to define particles, interactions, and parameters. All models — including the "built-in" ones — are UFO directories.
...
After setting masses, use [MadWidth](12_madwidth.md) to compute consistent widths:
...
**Details ->** [NLO Computations](06_nlo_computations.md) for loop-induced process syntax and options.
...
- The cross section depends **quadratically** on `ctr` (from the triangle amplitude squared) and has a **linear** interference term between triangle and box amplitudes. This makes the `σ(ctr)` dependence non-trivial — it is not a simple rescaling.
...
The `loop_sm` model contains the loop amplitude information (UV counterterms, R2 rational terms) needed for NLO calculations. Using `import model sm` with `[QCD]` syntax will fail. See [NLO Computations](06_nlo_computations.md).
...
- [Process Syntax](02_process_syntax.md) — `import model`, `generate`, `display` commands
- [Cards & Parameters](04_cards_and_parameters.md) — param_card for BSM parameters
- [MadWidth](12_madwidth.md) — computing BSM particle widths
- [NLO Computations](06_nlo_computations.md) — loop_sm model requirements
- [Advanced Syntax](17_advanced_syntax.md) — flavor schemes (4F vs 5F), coupling orders
- [Troubleshooting](16_troubleshooting.md) — slow FeynRules models, 0 diagrams
- [Decays & MadSpin](08_decays_and_madspin.md) — decay chain syntax, MadSpin for resonance decays
- [EW NLO Corrections](25_ew_nlo_corrections.md) — `loop_qcd_qed_sm` model for NLO EW

**Details ->** [EFT / SMEFTsim](26_eft_smeftsim.md)
