...
The width values must be consistent with the masses and couplings. Inconsistent widths cause problems with MadSpin (branching ratios > 1) and with the narrow-width approximation. Use [MadWidth](12_madwidth.md) (`compute_widths`) to calculate consistent widths.
...
```
230000  = lhaid    ! LHAPDF ID for the PDF set (default: nn23lo1)
```

The default uses an internal PDF set (`nn23lo1`, lhaid 230000). To use LHAPDF PDF sets, set `pdlabel = lhapdf` and specify the desired `lhaid`. Common LHAPDF IDs:
- `315000` = NNPDF31_lo_as_0118 (LO)
- `303400` = NNPDF31_nlo_as_0118 (NLO)
- `11000` = CT10nlo

See [Installation](01_installation_setup.md) for installing LHAPDF and PDF sets.

### Generation-Level Cuts

These cuts are applied during phase-space integration and directly affect the cross-section. They restrict the generated phase space to avoid soft/collinear divergences and speed up generation.
...
```
E_T(r) <= epsilon_gamma * pT_gamma * ((1 - cos(r)) / (1 - cos(R0)))^n
```
...
**Details ->** [Custom Cuts & Subprocess Grouping](../production_and_troubleshooting.md#custom-cuts-and-subprocess-grouping) | [Troubleshooting](16_troubleshooting.md)
...
**Warning**: Overly tight cuts can make the cross-section artificially small. See [Troubleshooting](16_troubleshooting.md) for diagnosing this.
...
See [Matching & Merging](07_matching_merging.md) for proper setup.
...
See [Systematics & Reweighting](13_systematics_reweighting.md) for full details.
...
**Details ->** [Lepton & Photon Colliders](22_lepton_photon_colliders.md) for other asymmetric beam setups (ep, eA)