...
With the defaults, **the standard 7-point scale envelope and PDF error-set reweighting are enabled automatically** — no user action is required.
...
For cross-sections dominated by **interference between amplitudes of different coupling orders** — such as SM–EFT interference terms isolated via squared-order syntax (e.g., `QCD^2==N QED^2==M`; see [Advanced Syntax](17_advanced_syntax.md)) — the on-the-fly scale reweighting via `use_syst` may produce unreliable uncertainty estimates.

**Recommended approach**: Estimate scale uncertainties for interference-dominated cross-sections by running separate event generations with different central scale choices (e.g., varying `scalefact` in the run_card to 0.5, 1.0, and 2.0 in independent runs). See [Scale Choices](20_scale_choices.md) for scale definition options.
...
- **Hessian** (e.g., CT10, MMHT): Error PDFs are eigenvector variations. Combine as:
  ```
  Δσ = sqrt( Σ (σ_i+ - σ_i-)² / 4 )
  ```

- **Monte Carlo replicas** (e.g., NNPDF): Each member is an equally likely PDF. The uncertainty is the standard deviation of cross-sections across replicas.
...
See [Output Formats & LHE](15_output_formats_lhe.md) for LHE parsing details.
...
**Details ->** [Parameter Scans](21_parameter_scans.md)
...
**Details ->** [Output Formats & LHE](15_output_formats_lhe.md)
...
- [Cards & Parameters](04_cards_and_parameters.md) — run_card systematics parameters
- [Scripted Execution](05_scripted_execution.md) — reweighting in scripts
- [NLO Computations](06_nlo_computations.md) — NLO scale uncertainties
- [Matching & Merging](07_matching_merging.md) — matching uncertainties
- [Output Formats & LHE](15_output_formats_lhe.md) — accessing rwgt weights in LHE
- [Parton Shower — Pythia8](09_parton_shower_pythia8.md) — shower modeling uncertainty
- [PDFs and LHAPDF](19_pdfs_and_lhapdf.md) — PDF sets for PDF uncertainty variations
- [Advanced Syntax](17_advanced_syntax.md) — squared-order syntax for isolating interference terms
- [Scale Choices](20_scale_choices.md) — scale definitions for scale variation systematics
