...
### "No Events Passed the Matching"

**Symptoms**: Pythia8 reports no events survived matching.

**Common causes:**

1. **ickkw = 1 without @N tags**: MLM matching requires multiplicity labels on each subprocess:
   ```
   # WRONG
   generate p p > t t~
...
2. **Single multiplicity with ickkw = 1**: Only generated one multiplicity but enabled matching. **Fix**: Set `ickkw = 0` or add more multiplicities.

3. **qCut too large**: If `JetMatching:qCut` is much larger than `xqcut`, most events are rejected. **Fix**: Use `qCut ≈ 1.2-1.5 × xqcut`.
...
See [Cards & Parameters — Custom Cuts](04_cards_and_parameters.md) for the `dummy_cuts` function signature and details.
...
At NLO, infrared (IR) poles from real-emission diagrams must cancel exactly against IR poles from virtual (loop) corrections and FKS subtraction counterterms. When they don't, MG5 aborts with this error. The cause is **always** a mismatch between the real and virtual contributions — never a PDF choice or parameter card issue (IR poles are properties of the matrix elements and subtraction scheme, not of PDFs).
...
**MG5 v3 vs v2**: MG5 v3 includes these pentagon contributions by default (controlled by `nlo_mixed_expansion = True` in the run_card), which can eliminate this specific pole cancellation failure. See [NLO Computations — Version Differences](06_nlo_computations.md).
...
MG5 v3 includes pentagon loops with non-colored particles that v2 discarded. This affects VBF, VBS, and single-top processes. Control with `nlo_mixed_expansion` in the run_card.
...
**Fix**: Run `compute_widths <particle>` to calculate the correct width, or manually set the DECAY value in the param_card. See [MadWidth](12_madwidth.md).
...
**Fix**: Create a restriction file that sets light quark masses to zero, then load with `import model MYMODEL-massless`. See [Models](03_models.md).

## Cross-Section Physics

Understanding expected values helps diagnose errors. See the cross-section hierarchy under "Cross-Section Too Large" above.

### W+/W- Asymmetry

At the LHC (pp collider), σ(W+) > σ(W-) with a ratio ≈ 1.3-1.4, reflecting the u/d PDF asymmetry in the proton.

### Energy Dependence

Cross-sections for heavy particles increase with √s. For pp → tt̄: σ(13 TeV) / σ(7 TeV) ≈ 4-5.

### VBF Isolation

`p p > h j j` mixes gluon-fusion + jets and VBF diagrams. To isolate VBF, use `QCD=0` and diagram filtering — see [Advanced Syntax](17_advanced_syntax.md). Do NOT use the `heft` model for VBF.

## Cross-References

- **[← Reference: Production & Troubleshooting](production_and_troubleshooting.md)**
- [Process Syntax](02_process_syntax.md) — `generate`, `check`, `display` commands
- [Cards & Parameters](04_cards_and_parameters.md) — all card parameters
- [Models](03_models.md) — model selection, restriction files
- [NLO Computations](06_nlo_computations.md) — NLO-specific errors
- [Matching & Merging](07_matching_merging.md) — matching diagnostics
- [Decays & MadSpin](08_decays_and_madspin.md) — width/decay issues
- [MadWidth](12_madwidth.md) — computing consistent widths
- [Advanced Syntax](17_advanced_syntax.md) — diagram filtering caveats
- [Installation & Setup](01_installation_setup.md) — LHAPDF, tool installation
- [Parton Shower — Pythia8](09_parton_shower_pythia8.md) — shower and matching issues
- [Output Formats & LHE](15_output_formats_lhe.md) — event format and weight issues
- [PDFs and LHAPDF](19_pdfs_and_lhapdf.md) — PDF-related errors and consistency
- [Width Treatment & Complex Mass Scheme](23_complex_mass_scheme.md) — width mismatches, BR > 1 errors
