...
MadGraph can export standalone code packages for evaluating matrix elements at arbitrary phase-space points without the full MadEvent event-generation infrastructure. This is the output mode to use when you need to call |M|² as a function — for reweighting, fitting, interfacing with external tools, or building custom integrators.
